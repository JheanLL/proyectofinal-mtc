...
-- PLATAFORMA WEB: GESTIÓN DE RUTAS TURÍSTICAS FERROVIARIAS Y PEATONALES
-- ARCHIVO ADJUNTO / ANEXO TÉCNICO: SCRIPT DDL Y SEEDS PARA AIVEN MYSQL
-- ==============================================================================
-- INSTRUCCIONES RÁPIDAS PARA COPIAR Y EJECUTAR:
-- 1. Abra este archivo de texto en cualquier editor (Bloc de notas, VS Code, etc.).
-- 2. Seleccione todo el contenido (Ctrl + A) y cópielo (Ctrl + C).
-- 3. Conéctese a su servidor MySQL (Aiven for MySQL Cloud, MySQL Workbench,
--    DBeaver o phpMyAdmin).
-- 4. Pegue el script en una nueva pestaña de consulta SQL y ejecútelo en bloque.
-- 5. El script se encargará de crear la base de datos 'bd_zonas_turisticas_mtc',
--    las 10 tablas relacionales con prefijos mandatorios 'tbl_', todos los índices
--    compuestos, las claves foráneas con eliminación en cascada y todos los datos
--    semilla (estaciones, zonas peatonales, horarios, tarifas y usuarios RBAC).
-- ==============================================================================
...
-- ==============================================================================
-- INSERCIÓN DE DATOS SEMILLA (SEEDS OFICIALES Y BANCO DE SIMULACIÓN MTC)
-- ==============================================================================
-- Se asegura la integridad referencial habilitando orden estricto de padres a hijos.
SET FOREIGN_KEY_CHECKS = 0;
...
VALUES 
  (1, 'admin@mtc.gob.pe', '$2b$10$.6Wq7zotY1GOnWyh9bt6COrSUGJoufV1LXJ6Vrbt0hfV2nv0qYDZ2', 'ADMIN', 'Administrador General MTC', TRUE),
  (2, 'operaciones@travelgroup.pe', '$2b$10$QxXVAJBC5h0LURn.lkxY8OeNw86UoJfdAbvy9sXJ/UUg2j2Schsse', 'TRAVEL_GROUP', 'Gestor Travel Group Perú', TRUE),
  (3, 'logistica@perurail.com', '$2b$10$jcbgDL3H6Yw96y04RSoeHOKYw2i9vsxTC58yIps4G4rUuXgVFxGA2', 'PERURAIL', 'Coordinador PeruRail', TRUE)
...
-- 3. SEED EN VIVO: PRONÓSTICOS METEOROLÓGICOS OFICIALES (SENAMHI / Open-Meteo)
-- Vinculado por Foreign Key con tbl_estacion(est_id)
...
('cli_est_01', 'est_01', CURDATE(), 4.5, 18.5, 7.3, 'Nublado', 71, 85, 2.5, 7.4, '{"nivel":"Verde","mensaje":"Condiciones normales de sierra.","recomendacion":"Llevar abrigo ligero y gorro."}', '["Ropa ligera transpirable","Lentes con filtro UV"]', 'SENAMHI EMA Cusco San Pedro (Satelital)', NOW()),
('cli_est_02', 'est_02', CURDATE(), 4.0, 18.0, 7.7, 'Nublado', 71, 84, 3.1, 8.0, '{"nivel":"Verde","mensaje":"Condiciones normales de sierra.","recomendacion":"Llevar abrigo ligero y gorro."}', '["Ropa ligera transpirable","Lentes con filtro UV"]', 'SENAMHI EMA Poroy (Satelital)', NOW()),
('cli_est_03', 'est_03', CURDATE(), 8.0, 22.0, 11.2, 'Nublado', 100, 78, 4.2, 9.3, '{"nivel":"Amarillo","mensaje":"Probabilidad alta de precipitaciones.","recomendacion":"Poncho impermeable y calzado con tracción."}', '["Poncho impermeable para lluvia","Calzado antideslizante"]', 'SENAMHI EMA Ollantaytambo (Satelital)', NOW()),
('cli_est_04', 'est_04', CURDATE(), 12.0, 24.0, 15.1, 'Lluvia Ligera', 100, 90, 2.0, 9.3, '{"nivel":"Amarillo","mensaje":"Probabilidad alta de precipitaciones en ceja de selva.","recomendacion":"Poncho impermeable y repelente de insectos."}', '["Poncho impermeable para lluvia","Repelente ecológico"]', 'SENAMHI EMA Machu Picchu Pueblo (Satelital)', NOW()),
('cli_est_05', 'est_05', CURDATE(), 7.5, 21.5, 11.0, 'Nublado', 100, 75, 3.5, 8.9, '{"nivel":"Amarillo","mensaje":"Probabilidad moderada de lluvias.","recomendacion":"Llevar paraguas y cortaviento."}', '["Cortaviento","Lentes de sol"]', 'SENAMHI EMA Urubamba (Satelital)', NOW()),
('cli_est_06', 'est_06', CURDATE(), 1.0, 15.0, 7.2, 'Nublado', 64, 65, 5.5, 9.5, '{"nivel":"Verde","mensaje":"Frío altiplánico y alta radiación solar.","recomendacion":"Protector solar FPS 50+ y casaca térmica."}', '["Casaca térmica","Protector solar FPS 50+"]', 'SENAMHI EMA Puno Titicaca (Satelital)', NOW())
ON DUPLICATE KEY UPDATE cli_temp_actual_c = VALUES(cli_temp_actual_c), cli_fecha_actualizacion = NOW();
